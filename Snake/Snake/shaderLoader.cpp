...
GLchar* shaderLoader::readShaderFile(const char *fileName){
...
GLuint shaderLoader::loadShader(GLenum shaderType, const char *fileName)
{
...
bool shaderLoader::SetShaderUniform1i(GLuint program, const char* name, GLint value){
...
bool shaderLoader::SetShaderUniform4f(GLuint program, const char* name, GLfloat* values){
	int location = glGetUniformLocation(program, name);
	if (location == -1){
		printf("Error getting location for uniform %s.\n", name);
		return false;
	}
...
bool shaderLoader::SetShaderUniformMatrix4f(GLuint program, const char* name, GLfloat* values){
	int location = glGetUniformLocation(program, name);
	if (location == -1)	{
		printf("Erro getting location for uniform %s.\n", name);
		return false;
